...
| `.github/workflows/release.yml` | What a tag does: installers for four targets, signed update artifacts, a **draft** release. Nothing is published until somebody has confirmed the previous version updates itself to it (§4). |
...
**Where things stand today:** Waves 0, 0.5, 0.6 and 1 are complete and merged, and `v0.2.0` is the first release anyone can install. There are two ways to use Booker and they share one layout engine: the `booker` command, and a desktop application that opens a book folder, edits it, draws its pages, reports what is wrong with it and exports a PDF — updating itself through a signed updater. An edit made outside the window (another editor, an agent, a branch checkout) is picked up and re-rendered. CI checks fmt, clippy, the test suite on macOS, Linux and Windows, the doc build, the TypeScript bindings, the dependency policy and the application's own lint, tests and bundle on every pull request, and the two tutorials teach everything Booker can currently do. **Wave 2 is the next one to start** — "write a simple book", `docs/PLAN.md` §8 — and it has no brief yet; writing `docs/waves/wave-2.md` is the first thing its lead does.
