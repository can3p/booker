...
**Where things stand today:** Waves 0, 0.5, 0.6 and 1 are merged. **Wave 1's release is the one step still outstanding**: nothing has been tagged yet, so there is no installer to download, and the application runs from a source checkout. The first release, `v0.2.0`, is cut next, followed by `v0.2.1` to prove that an installed copy updates itself (`CONTRIBUTING.md`, "Making a release"); Wave 2 starts once that check has passed. There are two ways to use Booker and they share one layout engine: the `booker` command, and a desktop application that opens a book folder, edits it, draws its pages, reports what is wrong with it and exports a PDF, with a signed updater built in that has not yet been exercised against a real release. An edit made outside the window (another editor, an agent, a branch checkout) is picked up and re-rendered. CI checks fmt, clippy, the test suite on macOS, Linux and Windows, the doc build, the TypeScript bindings, the dependency policy and the application's own lint, tests and bundle on every pull request, and the two tutorials teach everything Booker can currently do. **Wave 2 comes after the release** — "write a simple book", `docs/PLAN.md` §8 — and it has no brief yet; writing `docs/waves/wave-2.md` is the first thing its lead does.
