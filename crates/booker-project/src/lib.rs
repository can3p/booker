...
pub use booker_core::{BookConfig, Diagnostic, Error, ProjectRef, Result, Revision, Severity};
...
mod tests {
    ...
    fn a_project_relative_path_reads_the_same_on_every_platform() {
        ...
        // platform: this is the exact shape that made the Windows CI run red.
        let built: PathBuf = ["build", "the-secret-garden.pdf"].iter().collect();
        assert_eq!(display_path(&built), "build/the-secret-garden.pdf");
