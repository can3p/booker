...
- **The corepack trap came back in CI, wearing a different mask.** On the runner, `corepack enable pnpm` succeeded and then `pnpm install` died with `Error: Cannot find module /home/runner/.cache/node/corepack/v1/pnpm/12.5.1/bin/pnpm.cjs` — which reads as a corrupt cache, not as a corepack too old to fetch that pnpm. It is the same cause as the local `Cannot find matching keyid`, and the same fix: `npm install -g corepack@latest` before enabling pnpm, which `ci.yml` still does on Linux. **It does not hold on Windows**: the first tagged release build failed there with the same `Cannot find module …\corepack\v1\pnpm\12.5.1\bin\pnpm.cjs` after updating corepack, because the `pnpm` shim that answers belongs to the Node installation rather than to the corepack just installed. `release.yml` now uses `pnpm/action-setup`, which installs the version in `packageManager` directly and involves corepack not at all; if `ci.yml` ever runs the app job off Linux, it should do the same.
...
### A macOS watch can report files written just before it started
- Learned: 2026-09-21, Wave 1 / integration
- What: `notify` creates its FSEvents stream with `kFSEventStreamEventIdSinceNow`, so on paper nothing from before the watch can arrive. On the GitHub macOS runner it does. A test that created a book, started a watch on it and then rewrote thirty chapters was handed thirty-five paths: the thirty it wrote, plus `AGENTS.md`, `.gitignore`, `.gitattributes`, `assets/images/.gitkeep` and the template's own chapter — every file `Project::create` had written moments earlier. Not reproducible on an Apple Silicon laptop, idle or under heavy CPU and disk load; the runner is a virtual machine and its filesystem behaves differently.
- Why it matters: it failed two different tests on two different pushes and looked like two unrelated flakes. Any test that asserts *nothing* was reported, or that the *first* event is a particular one, is really asserting how fast the machine is. The shape that works, and that costs nothing, is a **sentinel**: a watch delivers in the order things happened, so writing a file and waiting for it to be reported proves everything written before it has been reported too. `app/src-tauri/tests/watching.rs` writes one after creating the fixture to drain its noise, and each test ends with another and collects until it arrives. Verified by simulating the leak — writing those same files after the watch starts makes four of the five old tests fail and none of the new ones.
- Where: `app/src-tauri/tests/watching.rs`; `notify` 8.2.0 `src/fsevent.rs` line 299.

### A Tauri bundle's version is the crate's, not the tag's
- Learned: 2026-09-21, Wave 1 / release
- What: when `tauri.conf.json` has no `version` key, Tauri takes the version of the `src-tauri` crate — here the workspace's `[workspace.package] version`. `tauri-action` writes the same number into `latest.json`, and the updater decides whether to offer an update by comparing that number with the installed one. The git tag names the GitHub release and nothing else.
- Why it matters: Wave 1 merged with the version still `0.1.0` and a release procedure that said "tag `v0.2.0`, then tag `v0.2.1`". Both builds would have reported `0.1.0` and the second would never have been offered — the one check `AGENTS.md` §4 says is never skipped would have failed for a reason unrelated to the updater. `release.yml` now has a `version` job that refuses a tag differing from `Cargo.toml` or `app/package.json` before any of the four builds start, and `CONTRIBUTING.md` lists the four places a bump touches.
- Where: `.github/workflows/release.yml` (`version` job), `Cargo.toml`, `app/package.json`, `CONTRIBUTING.md` "Making a release".

### Installed copies cannot see a draft release, so the update check happens after publishing
- Learned: 2026-09-21, Wave 1 / release
- What: the updater endpoint is `https://github.com/can3p/booker/releases/latest/download/latest.json`. GitHub resolves `releases/latest` to the newest *published*, non-prerelease release; drafts and prereleases are invisible to it. So an installed copy can only be offered a release that has already been published.
- Why it matters: the Wave 1 documents said a release would stay a draft until the update check had passed, which cannot happen with this endpoint. The procedure is now publish, check straight away from an installed previous version, and turn the release back into a draft if the check fails. Checking before anyone can receive the release would need a second endpoint (for example a fixed-URL manifest the check points at), which is the same machinery as the beta channel in `docs/OPEN-QUESTIONS.md` Q-12.
- Where: `app/src-tauri/tauri.conf.json` (`plugins.updater.endpoints`), `CONTRIBUTING.md` "Making a release".

### An unset secret is an empty string, and Tauri signs when the variable merely exists
- Learned: 2026-09-21, Wave 1 / release
