...
- **What the tests found that nothing else would have.** Three bugs and two bad tests, all in the watcher:
  - One write produces several filesystem events for the same path, so asking "is this our own echo?" once per event marked the first as ours and every later one as an outside edit. Every save turned into a reload that fought the editor. Deduplicating the paths within a burst fixed the common case and CI found the rest of it: the debouncer expires each event on its own clock, so on a loaded macOS runner one write arrived as two batches and the second was reported. Echo suppression now compares the file's content with what we wrote (`AGENTS.md` §7) and has no time window at all — which also means an outside edit landing a moment after a save is reported rather than swallowed.
