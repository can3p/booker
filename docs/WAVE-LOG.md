...
- Update check: **not done, by decision.** Distribution is paused (Q-13); the release workflow was proven by a hand-run dry run of all four targets, and the updater has never met a real release. The first release after the pause carries this check. A failed `v0.2.0` tag (Linux only) and its draft were deleted unpublished.
