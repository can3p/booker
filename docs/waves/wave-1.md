...
**Deviation, recorded here and in the wave log:** the in-application beta *channel* is deferred (`docs/OPEN-QUESTIONS.md` Q-12). A beta tag builds installers and a prerelease that no installed copy is offered, which is the useful half; staying on the channel needs a second manifest and a setting, and is a day's work whenever somebody actually wants a beta.
